function [b, m, nghb] = bp(E, phi, psi, obs, varargin)
% function [b, m, nghb] = bp(E, phi, psi, obs, varargin)
% 
% This function performs belief propagation in a forest, in other words, in
% a graph whose connected components are trees by first propagating all the 
% messages from the leaves all the way up to the root of each tree and then 
% backwards.
%
% INPUT
...
% EXAMPLES
%
% We show an example of bp in a graph whose connected components
% are trees.
% 
%       1             
%     /   \
...
% $ Date: 2013/10/16 09:45:43 $
    ...
    if length(comp)==1
        if nargin >= 4 && ~isempty(obs)
            [b, m] = bp_core(E, phi, psi, obs, 'root', root, 'nghb', nghb);
        else
            [b, m] = bp_core(E, phi, psi,  [], 'root', root, 'nghb', nghb);
        end
    else
        for i = 1:length(comp)
            E_cc    = E(edge2comp==i,:);
            ...
            if nargin >= 4 && ~isempty(obs)
                obs_cc       = obs(comp{i});
                [b_cc, m_cc] = bp_core(E_cc, phi_cc, psi_cc, obs_cc, 'nghb', nghb_cc);
            else
                [b_cc, m_cc] = bp_core(E_cc, phi_cc, psi_cc, 'nghb', nghb_cc);
            end
            b(comp{i}) = b_cc;
            m(comp{i}) = m_cc;
        end
