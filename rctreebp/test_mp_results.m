...
for t=1:Nt,
 ...
 T=rctreeMP(At,F);
 for tests=1:100, 
  Bel=maxproduct(At,F,T.roots);

  for i=1:size(At,1), 
    B2=Bel{i}(:); if (length(B2)==1), B2=ones(d,1)/d; end;
    if (any( (B2(:)-maxmarginal(T,i))./B2(:) > 1e-6 )) fprintf('%d ',i); return; end; 
    if (B2(T.map(i))-max(maxmarginal(T,i))>1e-6) fprintf('%d ',-i); return; end;
  end;
  ...
  for n=1:20, 
   ...
   if (add && length(T.roots)>1)
     ...
     indV=sort(indV);
     [T,Nchanged]=swapFactorMP(T,indF,indV,F{indF});
   else
     F{indF} = rand(d,1);
     At(:,indF)=zeros(N,1); At(indV,indF)=1;
     [T,Nchanged]=swapFactorMP(T,indF,indV,F{indF});
